...
PROCEDURE_MINIMUM_MINUTES = {
    ...
    "Psychosocial Rehab - Group": 26,
    "Psychosocial Rehabilitation Group": 26,
    "Individual Therapy": 16,
    "Assessment LPHA": 8,
    "Family Therapy - client present": 26,
    "Family Therapy - Client Present": 26,
    "Family Therapy - Client not Present": 26,
...
def read_county_services_invoiced(
    ...
    first_name = normalize_text(employee_name).split(",")[-1].strip()

    matching_names = county_staff_names[
    ...
    st.write("Possible County matches:", matching_names)
...
if services_file is not None and caseload_file is None:
    ...
    with center:
        if st.button("Run Productivity Only", use_container_width=True):
